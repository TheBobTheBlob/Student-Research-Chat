...
@router.post("/new")
async def new_message(message: messages.NewMessageRequest, current_user: models.users.UserRow = Depends(get_current_user)):
...
@router.post("/list")
async def list_of_message(message: messages.MessageListRequest, current_user: models.users.UserRow = Depends(get_current_user)):
